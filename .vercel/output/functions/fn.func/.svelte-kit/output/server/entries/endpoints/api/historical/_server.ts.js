...
const GET = async ({ url }) => {
  try {
    ...
    if (!symbolId)
      throw error(400, "searchParam symbolId missing.");
    const period1 = url.searchParams.get("period1");
    if (!period1)
      throw error(400, "searchParam period1 missing.");
    ...
    const data = {
      ...
      quote: {
        symbol: {
          ...
          name: quoteResult.displayName || quoteResult.longName || quoteResult.shortName || "unknown name",
          currency: quoteResult.currency || "unknown currency"
        },
        regularMarketTime: quoteResult.regularMarketTime || /* @__PURE__ */ new Date(),
